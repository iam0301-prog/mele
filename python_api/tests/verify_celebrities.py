...
from engines import astro, bazi, humandesign, maya, numerology, runes, tarot, ziwei
...
def main():
    ...
    for c in CELEBRITIES:
        ...
        if err:
            ...
            print(
                f"  靈數: {mark} LifePath={r['lifePath']} BirthDay={r['birthDay']} | 預期 LifePath={ref.get('numerology_lifepath')}"
            )
            if ok:
                pass_count += 1
            else:
        ...
        if err:
            ...
            o = r["oracle"]
            print(
                f"        指引={o['guide']['label']} | 支持={o['analog']['label']} | 挑戰={o['antipode']['label']} | 隱藏={o['occult']['label']}"
            )
            pass_count += 1
        ...
        r, err = safe_run(
            ...
            c["mi"],
            2,
            c["lon"] * 15 if c["lon"] < 200 else c["lon"],
        )
        if err:
            ...
            p = r["pillars"]
            print(
                f"  八字: 年{p['year'][0]}{p['year'][1]} 月{p['month'][0]}{p['month'][1]} 日{p['day'][0]}{p['day'][1]} 時{p['time'][0]}{p['time'][1]}"
            )
        ...
        if err:
            ...
            print(
                f"  紫微: 命宮={r['lifePalaceBranch']} 身宮={r['bodyPalaceBranch']} 五行局={r['fiveElementsBureau']}"
            )
            print("        \033[33m注意：本平台 ziwei 為簡化版（無 14 主星，僅 12 宮位排列）\033[0m")
            pass_count += 1

        # === Astro ===
        r, err = safe_run(
            astro.calculate, c["y"], c["mo"], c["d"], c["h"], c["mi"], c["tz"], c["lat"], c["lon"]
        )
        if err:
            ...
            print(
                f"  占星: ☉ {sun['zh']}座 {sun['degInSign']:.1f}° | ☽ {moon['zh']}座 {moon['degInSign']:.1f}° | ⬆ Asc {asc.get('zh', '-')}座"
            )
            print(
                f"        預期 Sun={ref.get('astro_sun')} Moon={ref.get('astro_moon')} Asc={ref.get('astro_asc')}"
            )
            pass_count += 1
    ...
    banner("總結")
    print(
        f"  \033[32m{pass_count} 通過\033[0m / \033[31m{fail_count} 失敗\033[0m / \033[33m{skip_count} 跳過\033[0m"
    )
    print()
