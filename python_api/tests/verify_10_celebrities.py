...
def run_one(c):
    section(
        f"⊹ {c['name']} ({c['year']}-{c['month']:02d}-{c['day']:02d} {c['hour']:02d}:{c['minute']:02d}{'男' if c['gender'] == '男' else '女'}) · 來源 {c['src']}"
    )
    exp = c["expected"]
    ...
    try:
        ...
        if "life_path" in exp:
            ...
            print(
                "靈數    ",
                check("numerology", c["name"], ok, f"生命靈數 {lp} {master} (預期 {exp['life_path']})"),
            )
        else:
    ...
    # ── 八字 ─────────────────────────
    try:
        br = bazi.calculate(c["year"], c["month"], c["day"], c["hour"], c["minute"], longitude=c["lon"])
        p = br["pillars"]
        ...
        print(
            f"        日主 {br['dayMaster']}({br['dayMasterYinYang']}{br['dayMasterWuxing']}) "
    ...
    try:
        ...
        if "sun_sign" in exp:
            ...
            print(
                "占星    ",
                check("astro", c["name"], ok, msg + f" (預期☉ {exp['sun_sign']} {'✓' if ok else '✗'})"),
            )
        else:
            print(f"占星     {msg}")
...
def main():
    print("═" * 78)
    print("  10 位名人 × 8 個算法引擎 多方位驗證")
    print(f"  Python {sys.version.split()[0]}")
    print("  資料來源 = python_api/engines/, python_api/data/")
    print("═" * 78)
