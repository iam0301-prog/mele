...
_SIGN_EN = ["Aries", "Taurus", "Gemini", "Cancer", "Leo", "Virgo", "Libra", "Scorpio", "Sagittarius", "Capricorn", "Aquarius", "Pisces"]
...
def render(data: dict, locale: str = "zh-TW") -> dict:
    ...
    sign_zh = ["牡羊", "金牛", "雙子", "巨蟹", "獅子", "處女", "天秤", "天蠍", "射手", "摩羯", "水瓶", "雙魚"]
    sign_labels = _SIGN_EN if is_en else sign_zh
    sign_sym = ["♈", "♉", "♊", "♋", "♌", "♍", "♎", "♏", "♐", "♑", "♒", "♓"]
    ...
    for i in range(12):
        ...
        sign_ring += f'<text x="{sx:.1f}" y="{sy + 18:.1f}" text-anchor="middle" font-size="9" fill="rgba(255,255,255,0.5)">{sign_labels[i]}</text>'
    ...
    planets_svg = ""
    for idx, (_key, p) in enumerate(planets.items()):
    ...
    _sign_zh_to_en = dict(zip(sign_zh, _SIGN_EN))

    def _sign_label(sign_dict: dict) -> str:
        zh_name = sign_dict.get("zh", "?")
        return _sign_zh_to_en.get(zh_name, zh_name) if is_en else zh_name

    sun_label = f"{_sign_label(sun_sign)} {sun_sign.get('degInSign', 0):.1f}°"
    moon_label = f"{_sign_label(moon_sign)} {moon_sign.get('degInSign', 0):.1f}°"
    asc_label = f"{_sign_label(asc_sign)} {asc_sign.get('degInSign', 0):.1f}°" if asc_sign else "—"
    mc_label = f"{_sign_label(mc_sign)} {mc_sign.get('degInSign', 0):.1f}°" if mc_sign else "—"

    svg = f"""
<svg viewBox="0 0 600 620" xmlns="http://www.w3.org/2000/svg">
{COMMON_KEYFRAMES}
{oracle_backdrop(600, 620, "Astrology Chart" if is_en else "占星星盤", "Natal Chart Oracle" if is_en else "本命星盤神諭")}
<defs>
...
<text x="{cx}" y="{cy - 40}" text-anchor="middle" font-size="11" fill="rgba(255,255,255,0.55)" letter-spacing="2">{"Your Natal Chart" if is_en else "你的本命星盤"}</text>
<text x="{cx}" y="{cy - 15}" text-anchor="middle" font-size="14" fill="{PALETTE["accent"]}" letter-spacing="3">{"Natal Chart" if is_en else "本命星盤"}</text>
<text x="{cx}" y="{cy + 10}" text-anchor="middle" font-size="11" fill="rgba(255,255,255,0.78)">☉ {sun_label}　☽ {moon_label}</text>
<text x="{cx}" y="{cy + 30}" text-anchor="middle" font-size="11" fill="rgba(255,255,255,0.7)">↑ {"Asc " if is_en else "上升 "}{asc_label}</text>
<text x="{cx}" y="{cy + 48}" text-anchor="middle" font-size="11" fill="rgba(255,255,255,0.6)">⊕ {"MC " if is_en else "天頂 "}{mc_label}</text>
</svg>"""

    if is_en:
        speech = (
            f"Your Sun is in {_sign_label(sun_sign)} at {sun_sign.get('degInSign', 0):.1f} degrees, "
            f"your Moon is in {_sign_label(moon_sign)} at {moon_sign.get('degInSign', 0):.1f} degrees, "
            f"and your Rising sign is {_sign_label(asc_sign)}."
        )
    else:
        speech = (
