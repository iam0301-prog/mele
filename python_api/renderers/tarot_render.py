...
def _suit_glyph(
    px: float, py: float, suit: str, style_key: str, uid: str, index: int, accent: str, soft: str
) -> str:
    if suit == "swords":
...
def _major_art(x: float, y: float, w: float, h: float, style_key: str, uid: str, card: dict) -> str:
    ...
    return motifs.get(
        name,
        f"""{star(cx, top + 34, 12)}<circle cx="{cx}" cy="{top + 54}" r="30" fill="none" stroke="{meta["accent"]}" stroke-width="2"/>""",
    )
...
def _card_art(
    x: float, y: float, w: float, h: float, style_key: str, uid: str, card: dict, draw: dict
) -> str:
    ...
    motif = (
        _major_art(x, y, w, h, style_key, uid, card)
        if arcana == "major"
        else _suit_art(x, y, w, h, style_key, uid, card)
    )
...
{cards_svg}
    ...
    for i, c in enumerate(cards):
        is_rev = c["position"] == "reversed"
        speech_parts.append(
            f"{labels[i] if i < len(labels) else f'第{i + 1}張'}：{c['card'].get('name_zh', c['card'].get('zh', ''))}{'逆位' if is_rev else '正位'}"
        )
