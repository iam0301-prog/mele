...
export const teacherCopies: Record<Locale, TeacherCopy> = {
  'zh-TW': {
    ...
    detail: {
      ...
      minutes: '分鐘',
      serviceNote: '預約前請確認這項服務符合你的問題類型。目前免費體驗，預約後可在「我的諮詢」查看狀態與管理紀錄。',
      betaFreeLabel: '免費體驗',
      guideBack: '回到引導',
      book: '預約',
    ...
    apply: {
      ...
      submitNoticeItems: ['7-14 天審核（含試講）', '審核期間我們會通過 Email 與你聯繫', '上架後可在後台自助管理時段與服務項目', '目前平台不收取任何費用，正式收費規則將另行通知'],
      submit: '送出申請',
      submitting: '送出中…',
    ...
    portal: {
      ...
      readiness: {
        ...
        body: '這裡把老師後台最容易漏掉的營運事項整理成清單，上線前可以逐項確認。',
        ok: 'OK',
        todo: '待確認',
        items: {
          profile: ['公開頁完整度', '名稱、頭銜、簡介與專長已具備，客人能快速判斷是否適合。', '請補齊頭銜、短介紹與專長，公開頁才會像一位可信任的老師。'],
          services: ['服務項目已設定', (count) => `目前有 ${count} 個上架服務，可承接預約。`, '請先請平台管理員設定服務名稱、時長與服務價格。'],
          bookings: ['預約處理節奏', (count) => `有 ${count} 筆諮詢需要留意，請在諮詢前主動確認問題。`, '目前沒有待處理預約，可以先檢查服務介紹與聯絡方式。'],
          testMode: ['費用說明', '目前所有預約均免費，先用完整流程熟悉預約與通知，不向客人收費。', '正式收費後，請先確認金流、取消政策與客服回覆節奏。'],
      ...
      memberBrief: {
        ...
        title: '會員詳解備忘',
        body: '會員前台先看簡易解釋；深入解釋、流日、流月、流年均可直接查看完整內容。老師端在諮詢前可先看會員提問與盤面脈絡，使解讀有本可循。',
        empty: '尚未有可讀取的會員盤面。待會員預約並附上提問或排盤資料後，這裡會整理成老師用備忘卡。',
        demoNote: '本機測試模式僅示範資料結構；正式上線後，後台會依預約與排盤紀錄銜接。',
      },
      assist: {
        aria: '輔助解盤工作台',
  ...
  en: {
    ...
    directory: {
      ...
      demoNotice: 'No approved production guides are available yet, so demo guides are shown. Production will automatically use approved Supabase guide records.',
    ...
    apply: {
      ...
      submitNoticeItems: ['Review takes 7-14 days including a trial session.', 'We will contact you by email during review.', 'After publishing, schedule and services can be managed in the guide workspace.', 'No platform fees at this time. Fee terms will be announced when billing opens.'],
    ...
    portal: {
      ...
      readiness: {
        ...
        body: 'This checklist gathers the operations most often overlooked before going live with real members.',
        ok: 'OK',
        todo: 'Check',
        items: {
          ...
          services: ['Services configured', (count) => `${count} active services are ready for booking.`, 'Ask an admin to set service name, duration, and pricing first.'],
          ...
          testMode: ['Fee status', 'All sessions are currently free. Use the full booking flow without charging members.', 'When billing opens, confirm payments, cancellation policy, and support response rhythm first.'],
      ...
      memberBrief: {
        ...
        body: 'Members see beginner readings first. Deeper readings, daily, monthly, and yearly transits are fully available. Guides can review the question and chart context before a session.',
        ...
        demoNote: 'Local test mode only demonstrates the data shape. Production will connect bookings and chart records.',
      },
      assist: {
    ...
    demoServices: {
      ...
      'service-1': { name: 'Tarot consultation', description: 'Demo service for testing guide workspace and pre-session reading assist.' },
      'service-2': { name: 'Bazi consultation', description: 'Demo service for testing guide workspace and member context briefs.' },
    },
  },
  vi: {} as TeacherCopy,
  id: {} as TeacherCopy,
...
teacherCopies.vi = inherit('vi', {
  ...
  portal: {
    ...
    readiness: {
      ...
      body: 'Checklist này gom các việc vận hành dễ bị bỏ sót trước khi ra mắt.',
      ok: 'OK',
      todo: 'Cần kiểm tra',
      items: {
    ...
    memberBrief: {
      ...
      body: 'Thành viên xem giải thích cơ bản trước. Giải thích sâu hơn, ngày lưu, tháng lưu, năm lưu đều có thể xem đầy đủ. Guide có thể xem câu hỏi và bối cảnh lá số trước buổi tư vấn.',
      ...
      demoNote: 'Chế độ thử cục bộ chỉ minh họa cấu trúc dữ liệu. Khi ra mắt sẽ kết nối với đặt lịch và hồ sơ lá số của thành viên.',
    },
    assist: {
...
teacherCopies.id = inherit('id', {
  ...
  portal: {
    ...
    readiness: {
      ...
      body: 'Checklist ini mengumpulkan hal operasional yang sering terlewat sebelum peluncuran.',
      ok: 'OK',
      ...
      items: {
        ...
        services: ['Layanan sudah diatur', (count) => `${count} layanan aktif siap menerima pemesanan.`, 'Minta admin untuk mengatur nama layanan, durasi, dan harga terlebih dahulu.'],
        ...
        testMode: ['Status biaya', 'Semua sesi saat ini gratis. Gunakan alur booking lengkap tanpa membebankan biaya ke member.', 'Ketika billing dibuka, konfirmasi pembayaran, kebijakan pembatalan, dan ritme dukungan terlebih dahulu.'],
    ...
    memberBrief: {
      ...
      body: 'Member melihat pembacaan dasar terlebih dahulu. Pembacaan lebih dalam, transit harian, bulanan, dan tahunan dapat diakses sepenuhnya. Guide bisa meninjau pertanyaan dan konteks bagan kelahiran sebelum sesi.',
      ...
      demoNote: 'Mode tes lokal hanya mendemonstrasikan struktur data. Saat diluncurkan akan terhubung dengan booking dan catatan bagan kelahiran member.',
    },
    assist: {
...
teacherCopies.ja = inherit('ja', {
  ...
  portal: {
    ...
    readiness: {
      ...
      body: '公開前に見落としやすい運用項目をまとめています。',
      ok: 'OK',
      todo: '要確認',
      items: {
        profile: ['公開プロフィール', '名前・肩書き・自己紹介・専門分野が揃っており、メンバーが適性を判断できます。', '肩書き・短い自己紹介・専門分野を追加して、公開ページを信頼できる内容にしてください。'],
        services: ['サービスの設定', (count) => `${count} 件のサービスが予約受付中です。`, 'まず管理者にサービス名・時間・価格の設定を依頼してください。'],
        bookings: ['予約への対応ペース', (count) => `${count} 件の相談がセッション前に要確認です。`, '待機中の予約はありません。サービス説明と連絡先を確認してください。'],
        testMode: ['費用について', '現在すべてのセッションは無料です。メンバーに費用をかけずに予約フロー全体を確認してください。', '有料モード移行時は、支払い・キャンセルポリシー・サポート対応のペースを事前に確認してください。'],
    ...
    memberBrief: {
      ...
      kicker: 'MEMBER CONTEXT',
      title: 'メンバーコンテキストメモ',
      body: 'メンバーはまず基本的な解説を確認します。より深い解説や流日・流月・流年の情報もすべて閲覧できます。ガイドはセッション前にメンバーの質問と命盤の文脈を確認できます。',
      empty: 'まだメンバーの命盤データがありません。メンバーが予約と質問・命盤データを送ると、ここにガイド用のサマリーカードが作成されます。',
      demoNote: 'ローカルテストモードではデータ構造のみを示しています。正式リリース後はメンバーの予約・命盤記録と連携します。',
    },
    assist: {
      aria: '解釈アシストワークスペース',
      kicker: 'READING ASSIST',
...
teacherCopies.ko = inherit('ko', {
  ...
  portal: {
    ...
    readiness: {
      ...
      body: '공개 전 놓치기 쉬운 운영 항목을 모았습니다.',
      ok: 'OK',
      todo: '확인 필요',
      items: {
        profile: ['공개 프로필', '이름, 직함, 소개, 전문 분야가 갖춰져 있어 회원이 적합도를 판단할 수 있습니다.', '직함, 짧은 소개, 전문 분야를 추가하여 공개 페이지가 신뢰감 있게 보이도록 해 주세요.'],
        services: ['서비스 설정', (count) => `${count}개의 활성 서비스가 예약을 받을 준비가 됐습니다.`, '관리자에게 서비스 이름, 시간, 가격 설정을 먼저 요청해 주세요.'],
        bookings: ['예약 처리 리듬', (count) => `${count}건의 상담이 세션 전 확인이 필요합니다.`, '대기 중인 예약이 없습니다. 서비스 설명과 연락처를 점검해 주세요.'],
        testMode: ['비용 안내', '현재 모든 세션은 무료입니다. 회원에게 비용 없이 전체 예약 흐름을 진행해 보세요.', '유료 모드로 전환 시 결제, 취소 정책, 고객 응대 리듬을 먼저 확인해 주세요.'],
    ...
    memberBrief: {
      ...
      kicker: 'MEMBER CONTEXT',
      title: '회원 맥락 요약',
      body: '회원은 먼저 기본 해석을 확인합니다. 더 깊은 해석, 일운·월운·연운도 모두 열람 가능합니다. 가이드는 세션 전 회원의 질문과 명반 맥락을 미리 확인할 수 있습니다.',
      empty: '아직 회원 명반 데이터가 없습니다. 회원이 예약하고 질문이나 명반 데이터를 첨부하면 여기에 가이드용 요약 카드가 생성됩니다.',
      demoNote: '로컬 테스트 모드는 데이터 구조만 보여줍니다. 정식 출시 후에는 회원의 예약 및 명반 기록과 연동됩니다.',
    },
    assist: {
      aria: '해석 보조 워크스페이스',
      kicker: 'READING ASSIST',
