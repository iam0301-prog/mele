...
export function DailyPointsClaim({
  ...
  const claim = async () => {
    ...
    if (error) {
      console.warn('claim daily points failed:', error.message);
      setNotice('今日儀式暫時無法完成，請稍後再試。');
      setBusy(false);
      return;
    }
      ...
      <div className={`rounded-2xl border border-accent-dim bg-accent/[0.06] p-4 text-sm leading-relaxed ${className}`}>
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="text-white/82">
            登入後可完成每日儀式，並查看延伸解讀（流日 / 流月 / 流年）。免費工具本身就完整。
          </div>
        ...
        <div className="text-white/82">
          {state.claimedToday ? (
            <span className="inline-flex items-center gap-1 text-success">
              <span aria-hidden="true">✓</span> 今日儀式已完成
            </span>
          ) : (
            <span className="text-white/64">今天還沒完成每日儀式</span>
          )}
